...
def main():
    parser = argparse.ArgumentParser(
        description="ORACLE 猪行为正负描述对比学习训练入口"
    )
    parser.add_argument("--train-data",          default="data/train-mp4.json")
    parser.add_argument("--val-data", "--test-data", dest="val_data", default="data/val.json", help="训练阶段用于模型选择的验证集 JSON；--test-data 为兼容旧脚本的别名")
    ...
    parser.add_argument("--motion-alpha",  type=float, default=0.5,
                        help="视频运动信息融合权重：feat = mean + alpha*std (0=纯mean)")
    ...
    logger.info(f"每视频帧数: {args.num_frames}，motion_alpha={args.motion_alpha}")

    logger.info("加载 CLIP ViT-B/32 ...")
    ...
    # ── 训练结束后最终评估均使用 eval 模式 ───────────────────────────────────
    logger.info("验证集零样本分类评估（margin 打分）...")
    # zero-shot 直接遍历 label_samples，不依赖 _flat，无需切换
    zero_shot = evaluate_zero_shot_classification_direct(
        model, val_dataset, device, save_dir=args.output_dir
    )

    logger.info("验证集全描述评估...")
    ...
    logger.info(f"零样本分类准确率: {zero_shot['overall_accuracy']:.2f}%")
    ...
    logger.info(f"完成！结果目录: {args.output_dir}")
