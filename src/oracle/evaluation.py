...
def evaluate_direct_model(
    ...
    """
    Evaluation function.
    Before calling this function, make sure the dataset has been switched to eval mode using set_eval_mode(),
    so that all positive and negative descriptions can be evaluated for each sample.
    """
    model.eval()
    total_loss = 0.0
    valid_batches = 0
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, desc=f"evaluate[{split_name}]"):
...
# ── Full-description validation evaluation ────────────────────────────────────────────────────────────
...
def evaluate_validation_full_descriptions(
    ...
    with torch.no_grad():
        for lbl, samples in tqdm(dataset.label_samples.items(), desc="Full-description validation evaluation"):
    ...
    logger.info("── Full-description validation evaluation results ──────────────────────────")
    ...
    if save_dir is not None:
        ...
        logger.info(f"Validation evaluation with all descriptions saved: {out_path}")
...
def evaluate_zero_shot_classification_direct(
    ...
    """
    Zero-shot multi-class evaluation.

    margin strategy:margin(label) = mean(pos_sim) - mean(neg_sim)

    This function directly iterates over label_samples and is independent of the _flat mode.
    """
    model.eval()
    feature_cache = dataset.feature_cache
    label_mapper  = dataset.label_mapper
    ...
    with torch.no_grad():
        for lbl, samples in tqdm(dataset.label_samples.items(), desc="Zero-shot evaluation"):
    ...
    logger.info("── Zero-shot per-class accuracy ──────────────────────────")
    ...
    if save_dir is not None:
        ...
        logger.info(
            f"Zero-shot error records have been saved: {os.path.join(save_dir, 'zero_shot_errors.json')}"
            f"{len(error_records)} records in total"
        )
