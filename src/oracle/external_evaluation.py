...
class DescriptionManager:
    ...
    def from_file(cls, path: str) -> "DescriptionManager":
        ...
        if not isinstance(descriptions, dict):
            raise ValueError("The top level of the description file must be a label-to-description dictionary mapping.")
        logger.info("Loaded %d labels from the description file: %s", len(descriptions), path)
    ...
    def get_positive(self, label: str) -> List[str]:
        ...
        if not values:
            raise ValueError(f"Label {label!r} is missing positive descriptions.")
        return values
    ...
    def get_negative(self, label: str) -> List[str]:
        ...
        if not values:
            raise ValueError(f"Label {label!r} is missing negative descriptions.")
        return values
...
def load_test_samples(
    ...
    skipped = 0
    unmatched = set()
    for item in tqdm(samples, desc="Loading test samples."):
    ...
    if unmatched:
        logger.warning("Test labels not found in the description set: %s", sorted(unmatched))
    logger.info(
        "Loaded test samples: %d entries, %d classes; skipped %d entries.",
...
def _project_texts(
    ...
    if missing:
        raise RuntimeError(f"Missing {len(missing)} text features. Please run text feature precomputation first.")
...
def evaluate_pos_neg_pair_accuracy(
    ...
    with torch.no_grad():
        for label, samples in tqdm(label_samples.items(), desc="Positive/negative pair accuracy"):
...
def evaluate_full_description_margin(
    ...
    skipped_media = 0
    with torch.no_grad():
        for label, samples in tqdm(label_samples.items(), desc="Full descriptions margin"):
            valid, missing = _valid_media_features(samples, media_features)
            skipped_media += missing
            pos_text = _project_texts(model, descriptions.get_positive(label), text_features, device)
            neg_text = _project_texts(model, descriptions.get_negative(label), text_features, device)
...
def evaluate_pos_neg_margin_accuracy(
    ...
    with torch.no_grad():
        for label, samples in tqdm(label_samples.items(), desc="Per-sample margin accuracy"):
            valid, missing = _valid_media_features(samples, media_features)
            skipped_media += missing
            pos_text = _project_texts(model, descriptions.get_positive(label), text_features, device)
            neg_text = _project_texts(model, descriptions.get_negative(label), text_features, device)
...
def evaluate_zero_shot_margin_classification(
    ...
    skipped_media = 0
    with torch.no_grad():
        for true_label, samples in tqdm(label_samples.items(), desc="Zero-shot classification accuracy"):
