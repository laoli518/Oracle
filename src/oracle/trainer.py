...
def train_direct_model(
    ...
    for epoch in range(num_epochs):
        ...
        g = torch.Generator()
        g.manual_seed(42 + epoch)   # 每个 epoch 有固定但不同的 shuffle 顺序
        ...
        if not epoch_losses:
            logger.error(f"Epoch {epoch+1}: 无成功步骤，终止训练")
            break
        ...
        logger.info(f"Epoch {epoch+1}: 切换数据集为 eval 模式进行评估...")
        ...
        if val_acc > best_val_acc:
            ...
            logger.info(f"  ✓ 新最佳验证准确率: {best_val_acc:.2f}%")
